(function () {
  ...
  function maxQuoteSequence() {
    var max = 0;
    for (var i = 0; i < quotes.length; i++) {
      var match = String(quotes[i].number || '').match(/^Q-(\d+)$/);
      if (match) max = Math.max(max, Number(match[1]) || 0);
    }
    return max;
  }

  function ensureQuoteSequence() {
    var stored = Number(settings.quoteSequence) || 0;
    var existing = maxQuoteSequence();
    if (existing > stored) settings.quoteSequence = existing;
  }

  function nextQuoteNumber() {
    ensureQuoteSequence();
    settings.quoteSequence = (Number(settings.quoteSequence) || 0) + 1;
    save();
    return 'Q-' + String(settings.quoteSequence).padStart(6, '0');
  }
  ...
  function show(id) {
    ...
    for (var j = 0; j < navs.length; j++) navs[j].classList.toggle('active', navs[j].getAttribute('data-screen') === id);
  ...
  function calc() {
    ...
    var materials = 0;
    for (var i = 0; i < amounts.length; i++) materials += Number(amounts[i].value) || 0;
  ...
  function addItem(description, amount) {
    ...
    row.querySelector('.item-amount').value = amount == null ? '' : amount;
    row.querySelector('.item-desc').addEventListener('input', calc);
    row.querySelector('.item-amount').addEventListener('input', calc);
    row.querySelector('button').addEventListener('click', function () { row.remove(); calc(); });
  ...
  function collectQuote() {
    ...
    return {
      ...
      number: currentQuote && currentQuote.number ? currentQuote.number : nextQuoteNumber(),
  ...
  function renderDashboard() {
    var total = 0;
    for (var i = 0; i < quotes.length; i++) total += Number(quotes[i].total) || 0;
  ...
  function renderPreview(q) {
    ...
    var businessContact = '';
    if (settings.businessAddress) businessContact += '<div>' + escapeHtml(settings.businessAddress) + '</div>';
    if (settings.businessPhone) businessContact += '<div>' + escapeHtml(settings.businessPhone) + '</div>';
    if (settings.businessEmail) businessContact += '<div>' + escapeHtml(settings.businessEmail) + '</div>';
    if (settings.abn) businessContact += '<div>ABN ' + escapeHtml(settings.abn) + '</div>';

    var customerContact = '';
    if (q.customerPhone) customerContact += '<div>' + escapeHtml(q.customerPhone) + '</div>';
    if (q.customerEmail) customerContact += '<div>' + escapeHtml(q.customerEmail) + '</div>';
    if (q.customerAddress) customerContact += '<div>' + escapeHtml(q.customerAddress).replace(/\n/g, '<br>') + '</div>';
  ...
  function editQuote(q) {
    ...
    $('customerAddress').value = q.customerAddress || '';
    ...
    $('lineItems').innerHTML = '';
    for (var i = 0; i < (q.items || []).length; i++) addItem(q.items[i].description, q.items[i].amount);
  ...
  function backup() {
    var blob = new Blob([JSON.stringify({version:7, quotes:quotes, settings:settings}, null, 2)], {type:'application/json'});
  ...
  function wire() {
    ...
    $('quoteFormEl').onsubmit = function (e) {
      ...
      var q = collectQuote();
      var index = -1;
      for (var i = 0; i < quotes.length; i++) if (quotes[i].id === editingId) index = i;
    ...
    $('settingsForm').onsubmit = function (e) {
      ...
      settings = {
        ...
        quoteSequence: Number(settings.quoteSequence) || maxQuoteSequence()
      };
      save();
      show('dashboard');
    };
    ...
    $('restoreFile').onchange = function (e) {
      ...
      reader.onload = function () {
        try {
          ...
          save();
          show('dashboard');
        } catch (err) { alert('That backup file could not be restored.'); }
      };
    ...
    for (var b = 0; b < back.length; b++) back[b].onclick = function () { show('dashboard'); };
    var navs = document.querySelectorAll('.nav');
  ...
  function start() {
    ...
    try {
      ...
      resetForm();
      show('dashboard');
      console.log('Tradie Quote Buddy runtime loaded v7');
    } catch (err) { console.error('Tradie Quote Buddy startup error:', err); }
  }
