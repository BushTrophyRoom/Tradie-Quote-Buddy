...
function installStyles(){if(document.getElementById('dusty-mobile-invoice-cleanup-style'))return;var s=document.createElement('style');s.id='dusty-mobile-invoice-cleanup-style';s.textContent='@media(max-width:800px){#invoices .invoice-card .invoice-open-btn,#invoices .invoice-card .tqb-invoice-bridge-open,#invoices .invoice-card .tqb-open-invoice-fallback,#invoices .tqb-invoice-bridge-card .invoice-open-btn,#invoices .tqb-invoice-bridge-card .tqb-invoice-bridge-open,#invoices .tqb-invoice-bridge-card .tqb-open-invoice-fallback{display:none!important}#invoices .invoice-card .tqb-mobile-delete-invoice,#invoices .tqb-invoice-bridge-card .tqb-mobile-delete-invoice{display:none!important}}';document.head.appendChild(s)}
function cleanCards(){
 document.querySelectorAll('#invoices .tqb-invoice-bridge-card,#invoices .invoice-card').forEach(function(card){
  ...
  card.querySelectorAll('.tqb-mobile-delete-invoice').forEach(function(b){b.remove()});
  card.querySelectorAll('button').forEach(function(b){if(b!==open)b.remove()});
  if(card.dataset.mobileInvoiceClick!=='1'){
   card.dataset.mobileInvoiceClick='1';card.style.cursor='pointer';
   card.addEventListener('click',function(e){if(e.target.closest&&e.target.closest('button'))return;var b=card.querySelector('.tqb-invoice-bridge-open,.invoice-open-btn,.tqb-open-invoice-fallback');if(b)b.click()});
  }
 })
}
...
function run(){if(!window.matchMedia||window.matchMedia('(max-width:800px)').matches){installStyles();removeQuickActions();cleanCards();addDelete()}}
