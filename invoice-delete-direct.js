...
function cleanCards(){document.querySelectorAll('.tqb-invoice-bridge-card').forEach(function(card){var open=card.querySelector('.tqb-invoice-bridge-open');if(open){open.style.display='none';open.setAttribute('aria-hidden','true')}card.querySelectorAll('button:not(.tqb-invoice-bridge-open)').forEach(function(b){b.remove()});if(card.dataset.mobileInvoiceClick!=='1'){card.dataset.mobileInvoiceClick='1';card.style.cursor='pointer';card.addEventListener('click',function(e){if(e.target.closest&&e.target.closest('.tqb-invoice-bridge-open'))return;var b=card.querySelector('.tqb-invoice-bridge-open');if(b)b.click()})}})}
