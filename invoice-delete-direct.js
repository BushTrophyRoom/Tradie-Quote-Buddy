...
function currentInvoiceId(){
  ...
  if(!view)return null;
  var card=view.querySelector('[data-invoice-id]');
  if(card)return card.getAttribute('data-invoice-id');
  var meta=view.querySelector('.invoice-meta b');
  var number=meta?(meta.textContent||'').trim():'';
  if(!number){
    var text=view.textContent||'';
    var m=text.match(/INV-\d+/);
    number=m?m[0]:'';
  }
...
function addDetailButton(){
  ...
  if(!actions)return;
  if(actions.querySelector('.tqb-delete-invoice'))return;
...
function addCardButtons(){
  var cards=document.querySelectorAll('.invoice-card[data-invoice-id]');
  for(var i=0;i<cards.length;i++){
    var card=cards[i];
    if(card.querySelector('.tqb-delete-invoice'))continue;
    var id=card.getAttribute('data-invoice-id');
    if(!id)continue;
    var b=document.createElement('button');
    b.type='button';b.className='tqb-delete-invoice';b.textContent='🗑️ Delete Invoice';
    b.style.marginTop='10px';b.style.width='100%';
    b.addEventListener('click',function(e){e.preventDefault();e.stopPropagation();deleteInvoice(this.getAttribute('data-id'))});
    b.setAttribute('data-id',id);
    card.appendChild(b);
  }
}
function ensureButtons(){style();addDetailButton();addCardButtons()}
function start(){
  document.addEventListener('click',function(e){
    var b=e.target.closest&&e.target.closest('.tqb-delete-invoice');
    if(b)return;
  },true);
