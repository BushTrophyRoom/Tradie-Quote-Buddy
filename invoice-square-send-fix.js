...
function create(inv){
 ...
 var k=owner();
 if(!/^[a-f0-9]{32,128}$/i.test(k))return Promise.resolve({ok:false,error:'Square is not connected on this device. Please reconnect Square in Settings.'});
...
function send(btn){
 ...
 cloud(inv).then(function(x){return x.squareCheckoutUrl?{ok:true,invoice:inv}:create(inv)}).then(function(result){
  if(!result.ok||!result.invoice.squareCheckoutUrl)throw new Error(result.error||'Square did not return a card payment link. The invoice was not sent.');
  inv=result.invoice;
 ...
 }).then(function(){btn.disabled=false;btn.textContent='✓ Invoice Sent';alert('Invoice '+number+' was emailed with the Square Pay by Card option.');setTimeout(function(){btn.textContent=old},2500)}).catch(function(e){btn.disabled=false;btn.textContent=old;alert('Invoice not sent. '+(e&&e.message?e.message:'Square could not create the card payment link.'))})
}
