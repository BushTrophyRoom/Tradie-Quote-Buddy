(function () {
  ...
  function buildResponseUrl(decision){
    ...
    try{var raw=localStorage.getItem('tqb_settings_v6'),settings=raw?JSON.parse(raw):{};businessEmail=String(settings.businessEmail||'').trim();if(!businessName||businessName==='Your Business Name')businessName=String(settings.businessName||'Tradie Quote Buddy');}catch(e){}
    if(q){
      ...
      for(var i=0;i<items.length;i++){
        var item=items[i]||{},qty=item.qty==null?1:Number(item.qty)||0,unit=item.unitPrice==null?Number(item.amount)||0:Number(item.unitPrice)||0,amount=qty*unit;
        if(item.description||amount||qty)lines.push(String(item.description||'')+' | Qty: '+qty+' | Unit: '+money(unit)+' | Amount: '+money(amount));
      }
    ...
    var payload={quote_number:quoteNumber,business_name:businessName,business_email:businessEmail,customer_name:customer.name,customer_phone:customer.phone,customer_email:customer.email,customer_address:customer.address,job_description:jobDescription,item_details:itemDetails,materials_details:materialsDetails,labour_details:labourDetails,subtotal:subtotal,discount:discount,gst:gst,total:total,quote_total:total,decision:decision,quote_status:decision,response_token:quoteNumber+'|'+customer.name+'|'+decision+'|'+total,responded_at:new Date().toLocaleString('en-AU')};
  ...
  function injectResponseActions(){var preview=$('quotePreview');if(!preview||!preview.querySelector('.paper-head'))return;var existing=patchExistingResponseControls(preview);if($(ACTION_ID))return;if(existing.accept||existing.decline)return;var wrap=document.createElement('div');wrap.id=ACTION_ID;wrap.className='customer-response';wrap.innerHTML='<div class="customer-response-title">Customer response</div><div class="customer-response-text">Ready to proceed? Choose <b>Accept Quote</b> or <b>Decline Quote</b>. Your response will be sent automatically to the business.</div><div class="customer-response-actions"><a class="response-accept" href="'+buildResponseUrl('Accepted')+'">✓ Accept Quote</a><a class="response-decline" href="'+buildResponseUrl('Declined')+'">✕ Decline Quote</a></div><div class="customer-response-note">Quote response • No payment is taken here</div>';preview.appendChild(wrap);}
  function start(){addStyles();var preview=$('quotePreview');if(!preview)return;var observer=new MutationObserver(function(){window.requestAnimationFrame(injectResponseActions);});observer.observe(preview,{childList:true,subtree:true});injectResponseActions();}
