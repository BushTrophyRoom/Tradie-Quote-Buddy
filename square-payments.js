...
function current(){var view=document.getElementById('invoiceView');if(!view)return null;var b=view.querySelector('.invoice-meta b');var n=b?(b.textContent||'').trim():'';if(!n){var m=(view.textContent||'').match(/INV-\d+/);n=m?m[0]:''}if(!n)return null;var list=invoices();for(var i=0;i<list.length;i++)if(String(list[i].number)===n)return list[i];return null}
...
function createLink(inv){
  return fetch(CREATE,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({owner_key:owner(),invoice_id:String(inv.id||''),invoice_number:String(inv.number||''),amount_cents:Math.round(Number(inv.total||0)*100),currency:'AUD',name:(inv.businessName||'Tradie Quote Buddy')+' - '+(inv.number||'Invoice')})})
  ...
  .then(function(x){
    ...
    save(list);
    inv.squareCheckoutUrl=x.data.checkout_url;
    inv.squarePaymentLinkId=x.data.payment_link_id||'';
    inv.squareOrderId=x.data.order_id||'';
    return inv;
  });
}
